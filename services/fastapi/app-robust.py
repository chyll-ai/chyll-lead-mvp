...
app.add_middleware(
    CORSMiddleware,
    allow_origins=[
        "https://frontend-8ogrwia2s-chyll.vercel.app",
        "https://frontend-946wwo1yb-chyll.vercel.app", 
        "http://localhost:3000",
        "http://localhost:5173",
...
try:
    ...
    DNS.timeout = DNS.lifetime = 1.0
except Exception as e:
    print(f"Warning: HTTP/DNS client initialization failed: {e}")
    HTTP = None
    DNS = None
...
def cset(k, v, ttl=3600*48): 
    CACHE[k] = {"val": v, "exp": time.time()+ttl}
...
def check_mx(domain):
    if not domain or not DNS: return 0
    k=f"mx:{domain}"
    v=cget(k)
...
def check_http_tls(domain):
    if not domain or not HTTP: return (0,0)
    k=f"http:{domain}"
    v=cget(k)
    if v is not None: return v
    http_ok, tls_ok = 0,0
    try:
        r=HTTP.get(f"https://{domain}")
        http_ok = 1 if r.status_code in (200,301,302,403) else 0
        tls_ok=1
    except:
        try:
            r=HTTP.get(f"http://{domain}")
            http_ok = 1 if r.status_code in (200,301,302,403) else 0
        except: 
            pass
    cset(k,(http_ok,tls_ok))
    return (http_ok,tls_ok)
...
def text_fingerprint(name, ape): 
    return f"{norm(name)} — {norm(ape)}".strip()
    ...
    try:
        ...
        for i in range(len(df)):
            ...
            out.append({
                ...
                "win_score": p, 
                "band": band,
                "neighbors": [{"name": f"similar_{j}", "sim": float(similarities[j]), "outcome": "won" if hist_labels[j]==1 else "lost"} for j in range(min(2, len(similarities)))],
                "reasons": [
